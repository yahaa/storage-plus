use std::{fs, path::{PathBuf, Path}, sync::Arc, time::{SystemTime, UNIX_EPOCH}};
...
#[post("/upload")]
async fn upload(mut payload: Multipart, data: web::Data<AppState>) -> actix_web::Result<impl Responder> {
    ...
    let file_path = data.storage_root.join(&key);
        // ensure storage dir
    tokio_fs::create_dir_all(&data.storage_root).await.map_err(|e| actix_web::error::ErrorInternalServerError(e.to_string()))?;
        let mut f = tokio_fs::File::create(&file_path).await.map_err(|e| actix_web::error::ErrorInternalServerError(e.to_string()))?;
        let mut size: i64 = 0;
        while let Some(chunk) = field.next().await {
            let bytes = chunk.map_err(|e| actix_web::error::ErrorBadRequest(e.to_string()))?;
            size += bytes.len() as i64;
            f.write_all(&bytes).await.map_err(|e| actix_web::error::ErrorInternalServerError(e.to_string()))?;
        }
...
#[get("/files/{key}")]
async fn download(path: web::Path<String>, data: web::Data<AppState>) -> actix_web::Result<NamedFile> {
    let key = path.into_inner();
    let key_db = key.clone();
    let repo = data.repo.clone();
    let meta_res = web::block(move || repo.get_by_key(&key_db)).await.map_err(|e| {
    ...
    Ok(file.set_content_disposition(actix_web::http::header::ContentDisposition {
        ...
        parameters: vec![actix_web::http::header::DispositionParam::Filename(String::from(meta.filename.clone()))],
    }))
}

#[delete("/files/{key}")]
async fn delete_file(path: web::Path<String>, data: web::Data<AppState>) -> actix_web::Result<impl Responder> {
    let key = path.into_inner();
    ...
    let meta = meta_res.map_err(|e| actix_web::error::ErrorInternalServerError(e.to_string()))?;
    if let Some(m) = meta {
        if let Err(e) = tokio_fs::remove_file(&m.path).await { error!("remove_file error: {}", e); }
...
async fn main() -> Result<()> {
    ...
    if let Some(parent) = args.db_path.parent() { fs::create_dir_all(parent)?; }
    ...
    let state = AppState { storage_root: args.storage_root.clone(), repo };
